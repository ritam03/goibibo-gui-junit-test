...
public class TrainSearchGUI extends JFrame {
    ...
    void searchTrains() {
        ...
        try {
            PreparedStatement statement = connection.prepareStatement("SELECT train_name FROM trains WHERE source=? AND destination=? AND departure_date=?");
            ...
            while (resultSet.next()) {
                trainList.add(resultSet.getString("train_name"));
            }

            if (trainList.isEmpty()) {
                resultArea.setText("No trains available for the given source, destination, and date.");
            } else {
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                new TrainSearchGUI(finalConnection).setVisible(true);
            }
        });
    }
}
